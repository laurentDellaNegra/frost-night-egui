...
// ---------------------------------------------------------------------------
// Track data (lots of them for a dense radar background)
// ---------------------------------------------------------------------------

/// Normalized track position (0..1) so tracks scale with window size.
struct Track {
    nx: f32,
    ny: f32,
    callsign: &'static str,
...
/// Live track state with animated positions.
#[derive(Clone)]
struct LiveTrack {
    ...
    ny: f32,
    callsign: &'static str,
    /// Velocity vector (pixels at reference 800×650 size, used for leader line).
    vx: f32,
    vy: f32,
    /// Normalized velocity per update (direction of travel).
    dnx: f32,
    dny: f32,
}
...
/// Normalized waypoint positions (0..1).
...
// ---------------------------------------------------------------------------
// Paint the radar background (egui shapes, drawn every frame)
// ---------------------------------------------------------------------------

/// Airspace / map area polygons (normalized coords).
const MAP_AREAS: &[(&[(f32, f32)], [u8; 3])] = &[
    // TMA region (lighter blue)
    (&[(0.10, 0.15), (0.35, 0.10), (0.45, 0.25), (0.40, 0.45), (0.15, 0.40)], [0x08, 0x12, 0x22]),
    // CTR zone (slightly brighter)
    (&[(0.50, 0.05), (0.75, 0.08), (0.80, 0.30), (0.60, 0.35), (0.48, 0.20)], [0x0A, 0x16, 0x2A]),
    // Restricted area (darker)
...
fn paint_background(ui: &egui::Ui, live_tracks: &[LiveTrack]) {
    ...
    // Tracks (live animated positions)
    ...
    // Waypoints (normalized coords → screen)
...
impl DemoApp {
    fn new(cc: &eframe::CreationContext) -> Self {
        ...
        // Convert pixel velocity to normalized step per 4-second update.
        // Reference size 800×650; each update advances by velocity / ref_size.
        let ref_w = 800.0f32;
        let ref_h = 650.0f32;
...
impl eframe::App for DemoApp {
    fn ui(&mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) {
        ...
        // Move tracks continuously along their velocity vector.
        let dt = ui.input(|i| i.stable_dt).min(0.1);
        let speed = 0.25; // scale factor (1.0 = one full velocity step per second)
        ...
        if self.card_open {
            ...
            ui.painter().rect_filled(
                card_rect,
                cr,
                egui::Color32::from_rgba_unmultiplied(r, g, b, drag_alpha),
            );
