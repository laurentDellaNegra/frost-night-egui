...
pub fn toolbar(
    ...
    toolbar_with_id(ui, theme, "toolbar", groups, selected, floating)
}

/// A vertical icon toolbar with a caller-provided ID salt.
pub fn toolbar_with_id(
    ui: &mut Ui,
    theme: &Theme,
    id_salt: impl Hash,
    groups: &[ToolbarGroup],
                ...
                for (gi, group) in groups.iter().enumerate() {
                    ...
                    if gi > 0 {
                        flex.add_ui(item(), |ui| {
                            ...
                            let (rect, _) =
                                ui.allocate_exact_size(Vec2::new(sep_w, h), Sense::hover());
