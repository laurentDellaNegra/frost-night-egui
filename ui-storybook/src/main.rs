            ...
            ui.horizontal(|ui| {
                ...
                ui.vertical(|ui| {
                    ...
                    match self.story.as_str() {
                        "accordion" => {
                            stories::accordion_story(ui, &self.theme, &mut self.accordion_state)
                        }
                        ...
                        "checkbox" => {
                            stories::checkbox_story(ui, &self.theme, &mut self.checkbox_state)
                        }
                        "toggle" => stories::toggle_story(ui, &self.theme, &mut self.toggle_state),
                        "segmented" => {
                            stories::segmented_story(ui, &self.theme, &mut self.segmented_state)
                        }
                        "badge" => stories::badge_story(ui, &self.theme, &mut self.badge_state),
                        "card" => stories::card_story(ui, &self.theme, &mut self.card_state),
                        "toolbar" => {
                            stories::toolbar_story(ui, &self.theme, &mut self.toolbar_state)
                        }
                        "separator" => {
                            stories::separator_story(ui, &self.theme, &mut self.separator_state)
                        }
                        "tabs" => stories::tabs_story(ui, &self.theme, &mut self.tabs_state),
                        "drag-card" => {
                            stories::drag_card_story(ui, &self.theme, &mut self.drag_card_state)
                        }
                        _ => {
